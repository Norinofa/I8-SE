...
*Speicherplatz un verschieden Versionen der Website zu speichern, dieser muss vom Dozenten zugänglich sein.
*SWFA-1* 

*Um das Speichern von inkonsistenten Daten zu verhindern soll bei dem Speichervorgang eine Überprüfung der eingegebenen Daten passieren. Auch mögliche Pflichtfelder müssen berücksichtigt werden.
*SWFA-4*

*Um faire Teams erstellen zu können, soll der Dozent die Möglichkeit haben, durch ein Eingabefeld festzulegen wieviele Personen in ein Team mindestens sein müssen. 
Hiergür muss der Algorithmus mit einer variablen Anzahl an Teams arbeiten können.
*SWFA-5*
...
* Die Nutzbarkeit muss gewährleistet sein, für den unwahrscheinlichen Fall falls 20 Nutzer gleichzeitig die Webseite nutzen.
Dies Performanz unserer Seite kann mittels einem Hosting über die Htw, als auch mit der Verminderung unnötigem Traffics gewährleistet werden.
*NFAP-1*, *NFAR-1*
...
Wenn wenn man die Website verlässt oder von dem Unterpunkt der Website navigiert, wird der Benutzer der Dozentenansicht gefragt ob die ausgefüllten Daten gespeichert werden sollen.

. Memory Management - Wiederherstellugn der Webseite
Um bei komplikationen oder Fehlern auf der Webseite soll es die Möglichkeit geben über einen Speicher eine bestimmte Versionen von einem ausgewählten Zeitpunkt wieder herzustellen.  

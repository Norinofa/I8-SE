...
(Die HTW Dresden führt im Modul Software Engineering realitätsnahe Projektarbeiten in Gruppen durch. Dafür muss eine Software entwickelt werden, die die Studierenden als ein Team bündelt. Dafür ist es wichtig, dass Präferenzen, Interessen und Fähigkeiten der Studierenden ausgewertet werden, sodass ein bestmögliches Team zusammengestellt werden kann.)

In diesem Projektplan der Gruppe I8 wird die Vorgehensweise und Organisation des Projektes protokolliert. Anhand des Open Unified Process wird das Projekt durch mehrere Iterationsschritte organisiert um bestmögliche Ergebnisse zu erzielen. Während des gesamten Projektes wird dieses Dokument stetig überarbeitet und dem Stand des Teams angepasst.
...
In der folgenden Tabelle werden die Teammitglieder und deren Rollenverteilung dargestellt.
